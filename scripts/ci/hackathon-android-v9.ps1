...
function Copy-TrustedSecrets {
  ...
  if ($env:EXPO_PUBLIC_QA_AUTOMATION -ne '1') { throw 'EXPO_PUBLIC_QA_AUTOMATION precisa ser 1 no QA Android.' }
  Write-Host 'Secrets locais carregados sem exibir valores; QA nativo habilitado.'
}
...
function Start-Metro {
  ...
  $cmd = "Set-Location '$mobile'; `$env:CI='true'; `$env:EXPO_PUBLIC_QA_AUTOMATION='1'; `$env:REACT_NATIVE_PACKAGER_HOSTNAME='127.0.0.1'; npx.cmd expo start --dev-client --localhost --port 8081 --clear *> '$metroLog'"
  ...
  if (-not $ready) {
    $tail = if (Test-Path $metroLog) { (Get-Content $metroLog -Tail 60) -join "`n" } else { 'metro.log ausente' }
    throw "Metro nao abriu 8081: $tail"
  }
}
...
function Wait-Text {
  ...
  do {
    ...
    if (Find-Node -Xml $xml -Candidates $Candidates) { return $true }
    $n++
...
function Assert-AppLoaded {
    ...
    try {
      ...
      if (Find-Node -Xml $xml -Candidates @('There was a problem loading the project.')) {
        ...
        foreach ($label in @('Hoje','Inbox','Radar','Planejar','Mais')) {
          if (-not (Find-Node -Xml $xml -Candidates @($label))) { $missing += $label }
        }
  ...
  try {
    ...
    $visible = @($diag.SelectNodes('//node') | ForEach-Object {
...
function Get-RevenueCatQaState {
  ...
  if (Find-Node -Xml $xml -Candidates @('QA RevenueCat Status configured=true isPro=true')) { return 'pro' }
  if (Find-Node -Xml $xml -Candidates @('QA RevenueCat Status configured=true isPro=false')) { return 'free' }
  return ''
}

function Wait-RevenueCatQaState {
  param([string]$Adb,[int]$Seconds=35,[string]$Name='qa-state')
  $deadline = (Get-Date).AddSeconds($Seconds)
  $n = 0
  do {
    Start-Sleep 2
...
function Test-RevenueCat {
  ...
  if (-not $state) { throw 'Status nativo de QA do RevenueCat nao ficou disponivel.' }
  ...
  if ($state -eq 'free') {
    ...
    do {
      try { $valid = Find-And-Tap -Adb $Adb -Candidates @('TEST VALID PURCHASE') -Name 'qa-valid-direct' } catch {}
      if ($valid) { break }

      $purchase = $false
      try {
        $purchase = Find-And-Tap -Adb $Adb -Candidates @('Test Store Purchase','Subscribe','Continue','Get Pro','Upgrade','Assinar','Purchase') -Name 'qa-purchase' -Scrolls 2
      } catch {}
      if ($purchase) { Start-Sleep 3 }

      try { $valid = Find-And-Tap -Adb $Adb -Candidates @('TEST VALID PURCHASE') -Name 'qa-valid-after' } catch {}
      if (-not $valid) { Start-Sleep 2 }
    } until ($valid -or (Get-Date) -gt $purchaseDeadline)

    if (-not $valid) { throw 'TEST VALID PURCHASE nao apareceu no paywall RevenueCat aberto pelo controle nativo QA.' }
    Start-Sleep 8
    $afterPurchase = Wait-RevenueCatQaState -Adb $Adb -Seconds 45 -Name 'qa-status-after-purchase'
    if ($afterPurchase -ne 'pro') { throw 'RevenueCat nao ativou Pro apos TEST VALID PURCHASE.' }
  }
  ...
  if ($persisted -ne 'pro') { throw 'RevenueCat nao manteve Pro apos fechar e reabrir o app.' }
  ...
  if ($restored -ne 'pro') { throw 'Restore Purchases concluiu, mas o entitlement Pro nao permaneceu ativo.' }
}

try {
  Set-Content -Path $logPath -Value "Hackathon Android QA v9 - $(Get-Date -Format o)" -Encoding utf8
